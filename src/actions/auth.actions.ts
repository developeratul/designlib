...
export async function checkUsernameAvailability(username: string) {
  ...
  const userQuery = await supabase.from("users").select("username").eq("username", parsedUsername.data).single()
...
export async function submitProfileDetails(data: z.infer<typeof profileDetailsFormSchema>) {
  ...
  await supabase.from("users").insert({ username, display_name, bio, avatarPath, isOnboarded: true })
}
